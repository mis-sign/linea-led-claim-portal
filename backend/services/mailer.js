...
async function sendEmail(db, { to, subject, html }) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 403 && msg.includes("own email")) {
        reason =
          "Resend's test sender can only deliver to the email you signed up with — it can't reach this customer's address yet. Verify your own domain at resend.com/domains (free, ~10 min) to send to any recipient.";
      } else if (res.status === 403 && msg.includes("domain")) {
...
function trackingLink(complaint) {
  ...
  if (!base) return null;
  return `${base.replace(/\/$/, "")}/?module=track&claimId=${encodeURIComponent(complaint.id)}&whatsapp=${encodeURIComponent(complaint.whatsappNumber)}`;
}

function trackingButton(link, label = "Track My Claim →") {
  if (!link) return "";
  return `<div style="text-align:center; margin-top:18px;">
    <a href="${link}" style="display:inline-block; background:linear-gradient(135deg,#ffab3d,#f2790a); color:#fff; text-decoration:none; font-weight:700; font-size:13.5px; padding:12px 26px; border-radius:10px;">${label}</a>
  </div>`;
}
...
function complaintRegisteredCustomerEmail(db, complaint) {
  ...
  const bodyHtml = `
    ...
    ${trackingButton(trackingLink(complaint))}
...
function customerRegistrationAdminEmail(db, account) {
  const company = db.admin.companyName;
  ...
  const bodyHtml = `
    <p style="text-align:center; color:#4d4d4f; font-size:14px; margin:0 0 6px;">
      A new customer wants access to their own dashboard. Review and verify them below.
    </p>
    ${detailTable([
    ...
    ${reviewLink ? trackingButton(reviewLink, "Review Registration →") : ""}
    <p style="color:#b3b4b6; font-size:11.5px; margin-top:16px; text-align:center;">
      Log in to the Admin Console → Customer Account Registrations to Verify or Reject.
    </p>`;
  return wrapTemplate({
    companyName: company,
    headerIcon: "👤",
    heading: "New Customer Registration",
    subheading: "Awaiting your approval",
    bodyHtml,
    footerNote: `Admin Console → Customer Account Registrations`
  });
}

function customerVerifiedEmail(db, account) {
  const company = db.admin.companyName;
  ...
  const bodyHtml = `
    <p style="text-align:center; color:#4d4d4f; font-size:14px; line-height:1.5; margin:0 0 6px;">
      Hi <strong>${account.customerName}</strong>, your account has been verified! You can now log in to see all of your warranty claims in one place.
    </p>
    <div class="detail-card" style="background:#fbfbfc; border:1px solid #f0f0f1; border-radius:12px; padding:14px 16px; margin-top:10px;">
      <p style="margin:0 0 8px; font-weight:700; color:#232426; font-size:13.5px;">How to log in:</p>
      <p style="margin:0 0 4px; color:#4d4d4f; font-size:13px;">1. Tap the button below</p>
      <p style="margin:0 0 4px; color:#4d4d4f; font-size:13px;">2. Enter your registered WhatsApp number: <strong>${account.phone}</strong></p>
      <p style="margin:0; color:#4d4d4f; font-size:13px;">3. Enter the 6-digit code sent to your WhatsApp</p>
    </div>
    ${dashboardLink ? trackingButton(dashboardLink, "Log In to My Dashboard →") : ""}`;
  return wrapTemplate({
    companyName: company,
    headerIcon: "✅",
...
module.exports = {
  ...
  customerRegistrationAdminEmail,
