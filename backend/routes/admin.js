...
// POST /api/admin/check-expiring-warranties - manually run the expiry check now (admin only)
router.post("/check-expiring-warranties", requireAdmin, async (req, res) => {
  try {
    const result = await checkExpiringWarranties();
    res.json({ message: `Checked ${result.checked} records — ${result.remindersSent} reminder(s) sent.`, ...result });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// ---- Photo storage settings (Cloudinary) ----
// Not secrets (cloud name / unsigned preset name are safe to expose), so no
// masking needed here unlike the notification credentials above.

// GET /api/admin/photo-storage (admin only)
router.get("/photo-storage", requireAdmin, async (req, res) => {
  const db = await readDb();
  res.json(db.admin.photoStorage || { cloudName: "", uploadPreset: "" });
});

// PUT /api/admin/photo-storage (admin only)
router.put("/photo-storage", requireAdmin, async (req, res) => {
  const db = await readDb();
  const { cloudName, uploadPreset } = req.body;
  if (!db.admin.photoStorage) db.admin.photoStorage = { cloudName: "", uploadPreset: "" };
  if (cloudName !== undefined) db.admin.photoStorage.cloudName = cloudName;
  if (uploadPreset !== undefined) db.admin.photoStorage.uploadPreset = uploadPreset;
  await writeDb(db);
  res.json({ message: "Photo storage settings updated." });
});

// POST /api/admin/photo-storage/test - upload a tiny test image via Cloudinary (admin only)
// Returns the real Cloudinary error if it fails, so you can see exactly
// what's wrong (wrong Cloud Name, preset not set to Unsigned, etc.)
// instead of guessing from a broken photo in a claim.
router.post("/photo-storage/test", requireAdmin, async (req, res) => {
  const db = await readDb();
  // A minimal valid 1x1 transparent PNG, just to verify the upload pipeline works.
  const tinyPngBuffer = Buffer.from(
    "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=",
    "base64"
  );
  const result = await cloudinaryUpload.uploadPhoto(db, tinyPngBuffer, "test.png");
  if (!result.uploaded) {
    const reason =
      result.reason === "not_configured"
        ? "Cloud Name and/or Upload Preset aren't saved yet — fill both in and save first."
        : result.reason;
    return res.status(400).json({ error: reason });
  }
  res.json({ message: "Test upload succeeded! Your Cloudinary setup is working.", url: result.url });
});
