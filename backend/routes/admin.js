...
// ---- Notification settings (WhatsApp via Maytapi + Email via Gmail SMTP) ----
...
router.get("/notifications", requireAdmin, (req, res) => {
  ...
  res.json({
    ...
    email: {
      ...
      gmailUser: n.email.gmailUser,
      gmailAppPassword: maskSecret(n.email.gmailAppPassword)
    }
  });
});
...
router.put("/notifications", requireAdmin, (req, res) => {
  ...
  if (email) {
    ...
    if (email.gmailUser !== undefined) db.admin.notifications.email.gmailUser = email.gmailUser;
    if (email.gmailAppPassword) db.admin.notifications.email.gmailAppPassword = email.gmailAppPassword;
  }
...
// Returns the real SMTP error if it fails, so you can see exactly what's wrong
// (wrong password, 2-Step Verification not enabled, etc.) instead of guessing.
