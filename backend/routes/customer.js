...
router.post("/register", async (req, res) => {
  ...
  // Notify the admin — fire-and-forget so the customer's response isn't
  // delayed by an email/WhatsApp call that might be slow.
  notifyAdminOfRegistration(db, account);
});

async function notifyAdminOfRegistration(db, account) {
  try {
    if (db.admin.supportEmail) {
      await mailer.sendEmail(db, {
        to: db.admin.supportEmail,
        subject: `New Customer Registration — ${account.customerName}`,
        html: mailer.customerRegistrationAdminEmail(db, account)
      });
    }
    ...
    if (adminNumber) {
      await whatsapp.sendWhatsApp(db, {
        ...
        message: whatsapp.customerRegistrationAdminMessage(db, account)
      });
    }
  } catch (err) {
    console.error("Registration notification error (non-fatal):", err.message);
  }
}
...
router.patch("/accounts/:id/verify", requireAdmin, async (req, res) => {
  ...
  // Notify the customer — fire-and-forget so the admin's response isn't delayed.
  notifyCustomerOfVerification(db, account);
});

async function notifyCustomerOfVerification(db, account) {
  try {
    if (account.email) {
      await mailer.sendEmail(db, {
        to: account.email,
        subject: `Your ${db.admin.companyName} account is verified`,
        html: mailer.customerVerifiedEmail(db, account)
      });
    }
    await whatsapp.sendWhatsApp(db, {
      toNumber: account.phone,
      message: whatsapp.customerVerifiedMessage(db, account)
    });
  } catch (err) {
    console.error("Verification notification error (non-fatal):", err.message);
  }
}
