...
// DELETE /api/complaints/clear-all - wipes ALL claims and resets the claim ID
// counter back to zero (admin only). Meant to be used ONCE, right after go-live,
// to remove the trial/test claims that were filed while setting up the portal —
// so the next claim filed starts fresh at LC-00001. This does NOT touch warranty
// records. This cannot be undone.
router.delete("/clear-all", requireAdmin, async (req, res) => {
  const db = await readDb();
  const removedCount = db.complaints.length;
  db.complaints = [];
  db.counters.complaint = 0;
  await writeDb(db);
  res.json({
    message: `Cleared ${removedCount} trial claim(s). The next claim filed will be LC-00001.`,
    removedCount
  });
});
