...
router.post("/", requireAdmin, async (req, res) => {
  ...
  record.warrantyId = record.warrantyId || (await nextId("WR"));
...
router.put("/:warrantyId", requireAdmin, async (req, res) => {
  const db = await readDb();
...
router.delete("/:warrantyId", requireAdmin, async (req, res) => {
  const db = await readDb();
...
router.post("/import", requireAdmin, upload.single("file"), async (req, res) => {
  ...
  // NOTE: IDs are generated by bumping db.counters.warranty in memory and
  // writing the whole document once at the end — NOT by calling nextId()
  // per row, which would mean one database round-trip per row (very slow
  // for a 500+ row CSV, and prone to write conflicts).
  rows.forEach((row, idx) => {
    ...
    if (!mapped.warrantyId) {
      db.counters.warranty += 1;
      mapped.warrantyId = `WR-${String(db.counters.warranty).padStart(5, "0")}`;
    }
