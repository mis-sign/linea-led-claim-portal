    ...
    admin: {
      ...
      notifications: {
        ...
        email: {
          enabled: false,
          resendApiKey: "",
          fromEmail: "" // leave blank to use Resend's built-in test sender (no domain setup needed)
        }
      }
    },
    counters: {
...
function readDb() {
  ...
  try {
    ...
    if (!data.admin.notifications) {
      ...
      let changed = false;
      if (data.admin.notifications.whatsapp.adminNumber === undefined) {
        data.admin.notifications.whatsapp.adminNumber = "";
        changed = true;
      }
      // Migration: older deployments used Gmail SMTP (gmailUser/gmailAppPassword).
      // That's replaced with Resend (resendApiKey/fromEmail) since Gmail's SMTP
      // ports get blocked on many hosting providers, including Render.
      if (data.admin.notifications.email.resendApiKey === undefined) {
        data.admin.notifications.email = {
          enabled: false,
          resendApiKey: "",
          fromEmail: ""
        };
