...
function buildDefaultData() {
  return {
    ...
    counters: { complaint: 0, warranty: 0, customerAccount: 0 }
  };
}
...
async function getCollection() {
  ...
  collectionPromise = (async () => {
    ...
    if (!existing) {
      ...
      if (!existing.customerAccounts) {
        existing.customerAccounts = [];
        changed = true;
      }
      if (existing.counters.customerAccount === undefined) {
        existing.counters.customerAccount = 0;
        changed = true;
      }
      if (changed) {
