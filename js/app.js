...
function openComplaintModal(c) {
  ...
  const photosHtml = (c.photos || [])
    .map((p) => `<a href="${photoUrl(p)}" target="_blank"><img src="${photoUrl(p)}" style="width:72px;height:72px;object-fit:cover;border-radius:8px;border:1px solid var(--border);margin:4px 6px 0 0;" /></a>`)
    .join("");
  const resolutionHtml = c.resolutionPhoto
    ? `<div class="detail-row"><span class="k">Resolution Photo</span></div>
       <a href="${photoUrl(c.resolutionPhoto)}" target="_blank"><img src="${photoUrl(c.resolutionPhoto)}" style="width:100px;height:100px;object-fit:cover;border-radius:8px;border:2px solid var(--success);margin-top:6px;" /></a>`
    : "";
  body.innerHTML = `
...
function renderTrackResult(c) {
  ...
  const photosHtml = (c.photos || [])
    .map((p) => `<img src="${photoUrl(p)}" style="width:60px;height:60px;object-fit:cover;border-radius:8px;border:1px solid var(--border);margin:4px 6px 0 0;" />`)
    .join("");
  ...
  panel.innerHTML = `
    ...
    ${c.resolutionPhoto ? `<div style="margin-top:12px;"><div class="hint" style="margin-bottom:6px;">Resolution Photo</div><img src="${photoUrl(c.resolutionPhoto)}" style="width:90px;height:90px;object-fit:cover;border-radius:8px;border:2px solid var(--success);" /></div>` : ""}
