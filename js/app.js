...
function openComplaintModal(c) {
  ...
  const photosHtml = (c.photos || [])
    .map((p) => `<a href="${API_BASE_URL}${p}" target="_blank"><img src="${API_BASE_URL}${p}" style="width:72px;height:72px;object-fit:cover;border-radius:8px;border:1px solid var(--border);margin:4px 6px 0 0;" /></a>`)
    .join("");
  const resolutionHtml = c.resolutionPhoto
    ? `<div class="detail-row"><span class="k">Resolution Photo</span></div>
       <a href="${API_BASE_URL}${c.resolutionPhoto}" target="_blank"><img src="${API_BASE_URL}${c.resolutionPhoto}" style="width:100px;height:100px;object-fit:cover;border-radius:8px;border:2px solid var(--success);margin-top:6px;" /></a>`
    : "";
  body.innerHTML = `
    <div class="detail-row"><span class="k">Claim ID</span><span class="v">${c.id}</span></div>
    <div class="detail-row"><span class="k">Branch / Site</span><span class="v">${c.branchName}</span></div>
    <div class="detail-row"><span class="k">Site Address</span><span class="v">${c.siteAddress}</span></div>
    <div class="detail-row"><span class="k">Warranty ID</span><span class="v">${c.warrantySerial}</span></div>
    <div class="detail-row"><span class="k">Warranty Status</span><span class="v">${warrantyStatus}</span></div>
    <div class="detail-row"><span class="k">Fault</span><span class="v">${c.faultType}</span></div>
    <div class="detail-row"><span class="k">Notes</span><span class="v">${c.notes || "—"}</span></div>
    <div class="detail-row"><span class="k">Contact</span><span class="v">${c.contactName}</span></div>
    <div class="detail-row"><span class="k">WhatsApp</span><span class="v">${c.whatsappNumber}</span></div>
    <div class="detail-row"><span class="k">Email</span><span class="v">${c.email || "—"}</span></div>
    <div class="detail-row"><span class="k">Status</span><span class="v">${c.status}</span></div>
    <div class="detail-row"><span class="k">Filed</span><span class="v">${formatDate(c.createdAt)}</span></div>
    ${c.location ? `<div class="detail-row"><span class="k">GPS</span><span class="v">${c.location.latitude.toFixed(5)}, ${c.location.longitude.toFixed(5)}</span></div>` : ""}
    <div style="margin-top:12px;">
      <div class="hint" style="margin-bottom:6px;">Fault Photos</div>
      ${photosHtml || '<span class="hint">No photos uploaded.</span>'}
    </div>
    <div style="margin-top:14px;">${resolutionHtml}</div>
  `;
...
function renderTrackResult(c) {
  const panel = document.getElementById("trackResultPanel");
  const badgeClass = c.status.replace(" ", "-");
  const warrantyBadgeClass = (c.warrantyStatus || "Not Found").replace(/ /g, "-");
  ...
  const photosHtml = (c.photos || [])
    .map((p) => `<img src="${API_BASE_URL}${p}" style="width:60px;height:60px;object-fit:cover;border-radius:8px;border:1px solid var(--border);margin:4px 6px 0 0;" />`)
    .join("");

  panel.style.display = "block";
  panel.innerHTML = `
    <div style="text-align:center; margin-bottom:14px;">
      <span class="badge ${badgeClass}" style="font-size:14px; padding:8px 20px;">${c.status}</span>
    </div>
    <div class="detail-row"><span class="k">Claim ID</span><span class="v">${c.id}</span></div>
    <div class="detail-row"><span class="k">Branch / Site</span><span class="v">${c.branchName}</span></div>
    <div class="detail-row"><span class="k">Fault</span><span class="v">${c.faultType}</span></div>
    <div class="detail-row"><span class="k">Warranty</span><span class="v">${c.warrantyStatus || "Not Found"}</span></div>
    <div class="detail-row"><span class="k">Filed</span><span class="v">${formatDate(c.createdAt)}</span></div>
    ${photosHtml ? `<div style="margin-top:12px;"><div class="hint" style="margin-bottom:6px;">Photos</div>${photosHtml}</div>` : ""}
    ${c.resolutionPhoto ? `<div style="margin-top:12px;"><div class="hint" style="margin-bottom:6px;">Resolution Photo</div><img src="${API_BASE_URL}${c.resolutionPhoto}" style="width:90px;height:90px;object-fit:cover;border-radius:8px;border:2px solid var(--success);" /></div>` : ""}
    <div style="margin-top:16px; border-top:1px solid var(--border); padding-top:10px;">
      <div class="hint" style="margin-bottom:6px;">Status Timeline</div>
      ${timeline}
    </div>
  `;
}
