...
// Customer Portal dropdown widget (top-right, separate from admin nav)
document.getElementById("custPortalToggle").addEventListener("click", (e) => {
  e.stopPropagation();
  document.getElementById("custPortalDropdown").classList.toggle("open");
});
document.getElementById("custPortalDropdown").addEventListener("click", (e) => {
  const btn = e.target.closest("button[data-module]");
  if (!btn) return;
  showModule(btn.dataset.module);
  document.getElementById("custPortalDropdown").classList.remove("open");
});
document.addEventListener("click", (e) => {
  const widget = document.getElementById("custPortalDropdown");
  if (widget.classList.contains("open") && !e.target.closest(".cust-portal-widget")) {
...
function showModule(name) {
  ...
  if (name === "mydashboard") initMyDashboard();
}
...
(async function handleDeepLink() {
  ...
  } else if (mod === "mydashboard") {
    document.body.classList.add("kiosk-mode");
    showModule("mydashboard");
  } else if (mod) {
    showModule(mod);
  }
})();
...
// ---- Clear Trial Data (admin only) ----
// One-time cleanup: wipes every claim on the dashboard and resets the claim
// ID counter, so the next real claim starts fresh at LC-00001. Warranty
// records are untouched. Guarded by a type-to-confirm prompt since this
// cannot be undone.
function updateClearTrialDataBtn() {
  const btn = document.getElementById("clearTrialDataBtn");
  btn.disabled = !adminToken;
  btn.style.opacity = adminToken ? "1" : "0.55";
  btn.title = adminToken ? "Permanently deletes every claim shown below." : "Admin login required";
}
document.getElementById("clearTrialDataBtn").addEventListener("click", async () => {
  if (!adminToken) return toast("Admin login required to clear trial data.", "error");
  const count = lastLoadedComplaints.length;
  if (count === 0) return toast("There's nothing to clear — the dashboard is already empty.", "success");
  const typed = prompt(
    `This will PERMANENTLY delete all ${count} claim(s) currently on the dashboard and cannot be undone.\n\nType DELETE to confirm.`
  );
  if (typed !== "DELETE") {
    if (typed !== null) toast("Cancelled — text didn't match \"DELETE\".", "error");
    return;
  }
  try {
    const data = await api("/api/complaints/clear-all", { method: "DELETE" });
...
document.getElementById("testPhotoStorageBtn").addEventListener("click", async () => {
  const btn = document.getElementById("testPhotoStorageBtn");
  btn.disabled = true;
  btn.textContent = "Testing…";
  try {
    const data = await api("/api/admin/photo-storage/test", { method: "POST" });
    toast(data.message, "success");
  } catch (err) {
    toast(err.message, "error");
  } finally {
...
// ================= MODULE 07: MY DASHBOARD (customer OTP login) =================
let custToken = null; // kept in memory only, same pattern as adminToken
let custPhone = "";
let custFaultChartInstance = null;
let lastLoadedCustClaims = [];

function initMyDashboard() {
  if (custToken) {
    document.getElementById("custLoggedOut").style.display = "none";
    document.getElementById("custLoggedIn").style.display = "block";
    loadMyClaims();
  } else {
    document.getElementById("custLoggedOut").style.display = "block";
    document.getElementById("custLoggedIn").style.display = "none";
  }
}

document.getElementById("custSendOtpBtn").addEventListener("click", async () => {
  ...
  btn.textContent = "Sending…";
  try {
    await api("/api/customer/request-otp", { method: "POST", body: JSON.stringify({ phone }) });
    custPhone = phone;
    document.getElementById("custPhoneStep").style.display = "none";
    document.getElementById("custOtpStep").style.display = "block";
    toast("Login code sent to your WhatsApp.", "success");
  } catch (err) {
    errEl.textContent = err.message;
  } finally {
    btn.disabled = false;
...
document.getElementById("showRegisterLink").addEventListener("click", (e) => {
  e.preventDefault();
  document.getElementById("custPhoneStep").style.display = "none";
  document.getElementById("custRegisterStep").style.display = "block";
  document.getElementById("custRegisterSuccess").style.display = "none";
  document.getElementById("custAuthError").textContent = "";
});
document.getElementById("showLoginLink").addEventListener("click", (e) => {
  e.preventDefault();
  document.getElementById("custRegisterStep").style.display = "none";
  document.getElementById("custPhoneStep").style.display = "block";
  document.getElementById("custAuthError").textContent = "";
});

document.getElementById("custRegisterBtn").addEventListener("click", async () => {
  const customerName = document.getElementById("regCustomerName").value.trim();
  ...
  if (!customerName || !/^[0-9]{10}$/.test(phone)) {
    errEl.textContent = "Customer name and a valid 10-digit WhatsApp number are required.";
    return;
  }
  const btn = document.getElementById("custRegisterBtn");
  btn.disabled = true;
  btn.textContent = "Submitting…";
  try {
    ...
    document.getElementById("custRegisterStep").style.display = "none";
    document.getElementById("custRegisterSuccess").style.display = "block";
...
document.getElementById("custResendOtpBtn").addEventListener("click", async () => {
  try {
    await api("/api/customer/request-otp", { method: "POST", body: JSON.stringify({ phone: custPhone }) });
...
document.getElementById("custVerifyOtpBtn").addEventListener("click", async () => {
  const code = document.getElementById("custOtp").value.trim();
  const errEl = document.getElementById("custAuthError");
  errEl.textContent = "";
  try {
    const data = await api("/api/customer/verify-otp", {
      method: "POST",
      body: JSON.stringify({ phone: custPhone, code })
    });
...
document.getElementById("custLogoutBtn").addEventListener("click", () => {
  ...
  custPhone = "";
  document.getElementById("custPhoneStep").style.display = "block";
  document.getElementById("custOtpStep").style.display = "none";
...
async function loadMyClaims() {
  try {
    const data = await customerApi("/api/customer/my-claims");
    lastLoadedCustClaims = data.complaints;
    renderMyDashboard(data.complaints, data.stats);
  } catch (err) {
    toast(err.message, "error");
  }
}

function renderMyDashboard(complaints, stats) {
  document.getElementById("custStatTotal").textContent = stats.total;
  document.getElementById("custStatPending").textContent = stats.pending;
  document.getElementById("custStatProgress").textContent = stats.inProgress;
  document.getElementById("custStatResolved").textContent = stats.resolved;

  const q = document.getElementById("custSearch").value.trim().toLowerCase();
  const statusFilter = document.getElementById("custStatusFilter").value;
  let visible = complaints;
  if (q) visible = visible.filter((c) => c.id.toLowerCase().includes(q) || c.branchName.toLowerCase().includes(q));
  ...
  if (visible.length === 0) {
    ...
    visible.forEach((c) => {
      ...
      viewBtn.textContent = "View";
      viewBtn.addEventListener("click", () => openComplaintModal(c));
      tr.querySelector(".row-actions").appendChild(viewBtn);
      body.appendChild(tr);
    });
  }
  ...
  const ctx = document.getElementById("custFaultChart");
  if (ctx && typeof Chart !== "undefined") {
    if (custFaultChartInstance) custFaultChartInstance.destroy();
    custFaultChartInstance = new Chart(ctx, {
      type: "bar",
      data: {
        labels: Object.keys(faultCounts).length ? Object.keys(faultCounts) : ["No data"],
        datasets: [{ data: Object.values(faultCounts).length ? Object.values(faultCounts) : [0], backgroundColor: "#f2790a", borderRadius: 6 }]
      },
...
document.getElementById("custRefreshBtn").addEventListener("click", loadMyClaims);
document.getElementById("custSearch").addEventListener("input", debounce(() => renderMyDashboard(lastLoadedCustClaims, {
  total: lastLoadedCustClaims.length,
  pending: lastLoadedCustClaims.filter((c) => c.status === "Pending").length,
  inProgress: lastLoadedCustClaims.filter((c) => c.status === "In Progress").length,
  resolved: lastLoadedCustClaims.filter((c) => c.status === "Resolved").length
}), 300));
document.getElementById("custStatusFilter").addEventListener("change", () => renderMyDashboard(lastLoadedCustClaims, {
  total: lastLoadedCustClaims.length,
  pending: lastLoadedCustClaims.filter((c) => c.status === "Pending").length,
  inProgress: lastLoadedCustClaims.filter((c) => c.status === "In Progress").length,
  resolved: lastLoadedCustClaims.filter((c) => c.status === "Resolved").length
...
// ---- Customer Account Registrations (Admin Console review panel) ----
async function loadCustomerAccounts() {
  try {
    const data = await api("/api/customer/accounts");
    renderCustomerAccounts(data.accounts);
  } catch (err) {
    toast(err.message, "error");
  }
}
...
function renderCustomerAccounts(accounts) {
  const body = document.getElementById("custAccountsTableBody");
  const empty = document.getElementById("custAccountsEmpty");
  body.innerHTML = "";
  ...
  accounts.forEach((a) => {
    const tr = document.createElement("tr");
    const badgeClass = a.status === "Verified" ? "Resolved" : a.status === "Rejected" ? "Out-of-Warranty" : "Pending";
    tr.innerHTML = `
      <td class="mono">${a.id}</td>
      <td>${a.customerName}<br><span class="hint">${a.address || ""}</span></td>
    ...
    if (a.status === "Pending") {
      ...
      verifyBtn.addEventListener("click", async () => {
        try {
          const data = await api(`/api/customer/accounts/${a.id}/verify`, { method: "PATCH" });
          toast(data.message, "success");
      ...
      rejectBtn.addEventListener("click", async () => {
        if (!confirm(`Reject ${a.customerName}'s registration?`)) return;
        try {
          const data = await api(`/api/customer/accounts/${a.id}/reject`, { method: "PATCH" });
          toast(data.message, "success");
          loadCustomerAccounts();
        } catch (err) {
          toast(err.message, "error");
      ...
      actionCell.innerHTML = '<span class="hint">—</span>';
    }
    body.appendChild(tr);
  });
}

document.getElementById("refreshCustAccountsBtn").addEventListener("click", loadCustomerAccounts);
